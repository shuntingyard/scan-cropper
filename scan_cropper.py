...
parser = argparse.ArgumentParser(
    description="Scanned image cropper."
...
def clipScans(img, candidates):
    ...
    for roi in candidates:
        ...
        try:
            scans.append(
                rotIm[min(y_vals) : max(y_vals), min(x_vals) : max(x_vals)]
            )
        except IndexError as e:
            print(
                "Error: Rotated image is out of bounds!\n"
                + "Try straightening the picture, and moving it away from the scanner's edge.",
                e,
            )
            global ERRORS
            ERRORS += 1
